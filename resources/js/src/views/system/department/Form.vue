            ...
            <v-card-text class="mt-4">
                ...
                <div :class="inputVisibility">
                    <v-form ref="form" :value="valid" lazy-validation class="mx-3">
                        ...
                        <v-text-field v-model="itemsValue.name" label="Name" :rules="$rules.required('Name')"
                            @input="changeToUpperCase('name')">
                        </v-text-field>
                        <v-autocomplete v-model="itemsValue.type" :items="departmentTypeList"
                            item-text="departmentTypeList" label="Type" :rules="$rules.required('Type')" return-object>
                        </v-autocomplete>
                        <v-autocomplete :color="dialogHeaderColor" v-model="employee"
                            :items="user.accountBranchType == 'MAIN' ? employeeList[branch!= null ? branch.name:'ALL']:employeeList.ALL"
                            item-text="fullName" label="Head" :loading="employeeList =='LOADING'? true: false"
